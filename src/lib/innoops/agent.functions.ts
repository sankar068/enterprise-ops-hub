...
function demoFallback(messages: { role: string; content: string }[]): AgentResponse {
  ...
  if (/(server (is )?down|outage|all employees)/i.test(msg)) {
    return {
      ...
      requestType: "Major Incident",
      intentConfidence: 0.98,
      title: "Company-wide server outage",
      description: "Production server reported down affecting all employees.",
      priority: "Critical",
      extractedDetails: {
        issueCategory: "Infrastructure",
        deviceOrSoftware: "Server",
        issueDescription: lastUser,
        businessImpact: "Org-wide outage",
        urgency: "Immediate",
      },
      missingFields: [],
      readyToCreate: true,
      nextAction: "Show request preview",
      responseToUser:
        "This looks like a critical org-wide outage. I've prepared a ticket for immediate escalation. Please confirm to submit.",
    };
  }
  if (/(camera|wi-?fi|laptop|password|printer|software|crash|not working)/i.test(msg)) {
    ...
    return {
      department: "IT",
      intent: "create_request",
      requestType: "IT Support Ticket",
      intentConfidence: 0.92,
      title: "IT issue reported",
      description: lastUser,
      priority: high ? "High" : "Normal",
      extractedDetails: {
        issueCategory: "Hardware/Software",
        deviceOrSoftware: "Laptop",
        issueDescription: lastUser,
        businessImpact: high ? "Blocks urgent meeting" : "Productivity impact",
        urgency: high ? "High" : "Normal",
      },
      missingFields: [],
      readyToCreate: true,
      nextAction: "Show preview with troubleshooting",
      responseToUser:
        "I've drafted an IT ticket. Quick checks first: restart the app, verify permissions, reboot the device. Confirm below to submit the ticket.",
    };
  }
  if (/(leave|vacation|day off|casual|sick)/i.test(msg)) {
    ...
    return {
      department: "HR",
      intent: ready ? "create_request" : "clarify",
      requestType: "Leave Request",
      intentConfidence: 0.95,
      title: "Casual Leave Request",
      description: lastUser,
      priority: "Normal",
      extractedDetails: ready
        ? {
      ...
      readyToCreate: ready,
      nextAction: ready ? "Show request preview" : "Ask for dates and reason",
      responseToUser: ready
        ? "Your leave request is ready for review."
        : "Sure — what leave type, what dates (start and end), and what's the reason?",
    };
  }
    ...
    return {
      ...
      extractedDetails: ready
        ? {
            expenseType: "Travel",
            amount,
            currency: /\$/.test(msg) ? "USD" : "INR",
            expenseDate: msg.match(
      ...
      readyToCreate: ready,
      nextAction: ready ? "Show request preview" : "Ask for amount, date, purpose, receipt",
      responseToUser: ready
        ? "Your Finance claim is ready for review."
        : "Got it. What was the amount, the date, the purpose, and do you have the receipt?",
    };
  }
  if (/(pending|status|track|my request)/i.test(msg)) {
    return {
      ...
      missingFields: [],
      readyToCreate: false,
      nextAction: "Show user's requests",
  ...
  return {
    ...
    responseToUser:
      "Could you clarify? I can help with HR leave, Finance reimbursements, or IT support tickets.",
  };
}
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      return {
        ok: true as const,
        mode: "demo" as const,
        result: demoFallback(data.messages),
        warning: "Foundry unavailable, using Demo Mode.",
      };
    }
  });
